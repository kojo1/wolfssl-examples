...
int main(int argc, char** argv)
{
    ...
    /* Get the right encoded password length */
    ...
    /* Get the right receiver mail address length */
    for(len = 0;len <sizeof(plain);len++){
        if((plain[len] == '\n') || (plain[len] == ' ')) break;
    }
    /* Compose the receiver mail address */
    memset(buff,0,sizeof(buff));
    strcpy(buff,oversslCmd[9]);
    memcpy(buff+strlen(oversslCmd[9]),plain,len);
    strcat(buff,">\r\n");
    printf("%s\n", buff);
    ...
    /* Compose the reveiver mail address */
    memset(buff,0,sizeof(buff));
